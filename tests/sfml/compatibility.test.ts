...
describe("legacy SFML compatibility", () => {
  ...
  test.each(guideBaseline)(
    "guide: $path",
    async ({ path, status, warnings }) => {
...
describe("SFML diagnostics and large inputs", () => {
  ...
  test("deep expressions and blocks do not exhaust the call stack", () => {
    expect(
      validateSyntax(
        wrap(
          `IF ${"(".repeat(10_000)}${"NOT ".repeat(10_000)}TRUE${")".repeat(10_000)} THEN END`,
        ),
      ).ok,
    ).toBe(true);
    expect(
    ...
    expect(
      validateSyntax(
        wrap(
          `INPUT WITH ${"(".repeat(10_000)}#forge:ingots${")".repeat(10_000)} FROM a`,
        ),
      ).ok,
    ).toBe(true);
  });
  ...
  test("keeps required, length, and control-character validation", () => {
    ...
    expect(
      analyzeSfmlCode(" ", { required: true, emptyMessage: "Paste code" })
        .message,
    ).toBe("Paste code");
