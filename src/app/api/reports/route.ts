...
const reasonOptions = [
  ...
  {
    value: "promotes_violence_or_dangerous_behavior",
    reason: ReportReason.PROMOTES_VIOLENCE_OR_DANGEROUS_BEHAVIOR,
  },
...
const reportSchema = z
  ...
  .refine(
    (data) => data.reason !== "other" || Boolean(data.message && data.message.trim().length >= 10),
    {
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Include a message between 10 and 500 characters." },
      { status: 400 },
    );
  }
  ...
  const selectedReason =
    reasonOptions.find((option) => option.value === reasonValue)?.reason ?? ReportReason.OTHER;
  if (type === "post") {
  ...
  if (type === "post" && postId) {
    const [distinctOpenReports, postStatus] = await Promise.all([
      ...
      db.post.findUnique({
        where: { id: postId },
        select: { isDeleted: true, deletionFlaggedByAuto: true },
      }),
    ]);
